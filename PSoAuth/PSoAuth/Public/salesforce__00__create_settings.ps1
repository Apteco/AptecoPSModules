...
$settings = @{
    ...
    "taskDefaultName" = "Apteco Salesforce Token Refresher"
...
################################################
 ...
 If ( $createTask -eq "0" ) {
    ...
    $taskParams = [Hashtable]@{
        TaskPath = "\Apteco\"
        TaskName = $taskname
        Description = "Refreshes the token for Salesforce because it is only valid for 2 hours"
        Action = New-ScheduledTaskAction -Execute "$( $settings.powershellExePath )" -Argument "-ExecutionPolicy Bypass -File ""$( $scriptPath )\salesforce__05__check_token.ps1"""
