[![License](https://img.shields.io/badge/License-MIT-blue.svg)](https://github.com/Apteco/AptecoPSModules/blob/main/LICENSE)
...
The module successor of `Install-Dependencies`. Installs modules and local/global NuGet packages via PowerShellGallery/NuGet, and reuses `Get-PSEnvironment` from `ImportDependency` to skip anything already installed at a current version.

```PowerShell
Install-Dependency -Module "WriteLog" -LocalPackage "System.Data.SQLite", "Npgsql" -Verbose
```

It can also check for and install the Visual C++ Redistributable (x64), needed by DuckDB and other native NuGet packages
