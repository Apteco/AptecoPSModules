        ...
        try {
            ...
            If ( ( Test-Path -Path $Path -IsValid ) -eq $true ) {
                ...
                If (( Test-Path -Path $Path ) -eq $true) {
                    ...
                    try {
                        $joinedSettings = Merge-PSCustomObject -Left $Script:defaultSettings -Right $settings -AddPropertiesFromRight -MergePSCustomObjects -MergeArrays -MergeHashtables
                    } catch {
                        Write-Error -Message "Settings cannot be joined"
                    }
